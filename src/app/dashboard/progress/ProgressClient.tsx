  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 800 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 18 }}>
        {[
          { label: 'Examens passés', val: String(examSessions.length), color: '#2563EB' },
          { label: 'Score moyen', val: hasData ? avgScore + '%' : '—', color: avgScore >= 70 ? '#15803D' : '#B91C1C' },
          { label: 'Meilleur score', val: hasData ? bestScore + '%' : '—', color: '#334155' },
          { label: 'Examens réussis', val: hasData ? `${passCount}/${examSessions.length}` : '—', color: '#64748B' },
        ].map(m => (
      ...
      {/* Graphique scores */}
      <div style={{ background: 'var(--surface-2)', border: '0.5px solid var(--border)', borderRadius: 14, padding: '16px', marginBottom: 14 }}>
        ...
        {!hasData ? (
          ...
          <>
            <div style={{ position: 'relative', height: chartH + 20 }}>
              <svg width="100%" height={chartH + 20} viewBox={`0 0 ${chartW} ${chartH + 20}`} preserveAspectRatio="none" style={{ overflow: 'visible' }}>
                ...
                {[0, 25, 50, 70, 75, 100].map(v => (
                  <g key={v}>
                    <line x1="20" y1={scoreToY(v)} x2={chartW - 10} y2={scoreToY(v)}
                      stroke={v === 70 ? '#B91C1C' : 'var(--border)'}
                      strokeWidth={v === 70 ? 1.5 : 0.5}
                      strokeDasharray={v === 70 ? '5,4' : '0'} />
                    <text x="15" y={scoreToY(v) - 3} fontSize="9" fill={v === 70 ? '#B91C1C' : 'var(--text-muted)'} textAnchor="end">{v}%</text>
                  </g>
                ))}
                {/* Area fill */}
                {pts.length > 1 && (
                ...
                {pts.map((s, i) => {
                  ...
                  return (
                    <g key={i}>
                      <circle cx={x} cy={y} r="5" fill={s.passed ? '#15803D' : '#B91C1C'} />
                      <text x={x} y={y - 9} fontSize="9" fill="#2563EB" textAnchor="middle" fontWeight="600">{s.score}%</text>
                    </g>
                  )
                })}
              </svg>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 8, fontSize: 11, color: 'var(--text-muted)' }}>
              <span>● <span style={{ color: '#15803D' }}>Réussi</span></span>
              <span>● <span style={{ color: '#B91C1C' }}>Échoué</span></span>
              <span style={{ color: '#B91C1C' }}>— Seuil 70%</span>
            </div>
          </>
        )}
      </div>
      ...
      {domainStats.length > 0 && (
        <div style={{ background: 'var(--surface-2)', border: '0.5px solid var(--border)', borderRadius: 14, overflow: 'hidden', marginBottom: 14 }}>
          ...
          {domainStats.map((d, i) => {
            ...
            const barColor = d.score >= 75 ? '#15803D' : d.score >= 60 ? '#2563EB' : '#B91C1C'
            return (
      ...
      {recommendations.length > 0 && (
        <div style={{ background: '#FFFBEB', border: '0.5px solid #B45309', borderRadius: 14, padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: '#B45309', marginBottom: 8 }}>
          ...
          <ul style={{ paddingLeft: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {recommendations.slice(0, 3).map((r, i) => (
              <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 12, color: '#334155' }}>
                <span style={{ width: 16, height: 16, borderRadius: '50%', background: '#B45309', color: '#fff', fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1, fontWeight: 600 }}>{i + 1}</span>
                {r}
              </li>
            ))}
          </ul>
